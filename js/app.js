/**
 * AETHERIA - Master Numerology & Astrological Synthesis Main Controller (Modular)
 */

import { PRESET_PROFILES } from "./data/presets.js";
import { CITIES_DATABASE } from "./data/cities.js";
import { CORE_NUMBERS_DATA, KARMIC_DEBTS_DATA, CHALLENGES_DATA, PINNACLES_DATA } from "./data/interpretations.js";
import { SOLFEGGIO_FREQUENCIES, NUMBER_REMEDIES, LUNAR_MANSIONS } from "./data/remedies.js";
import { calculateFullBlueprint } from "./numerology.js";
import { calculateAstrologyProfile } from "./astrology.js";
import { calculatePersonalCycles, generateYearlyTransitRoadmap } from "./forecasting.js";
import { synthesizeLifePathSun, synthesizeSoulUrgeMoon, synthesizePersonalityAscendant, generateLifeMandate } from "./synthesis.js";
import { calculateLoShuGrid, calculatePlanesOfExpression, calculateBridgeNumbers, calculateThreeLifePeriods, LOSHU_LAYOUT } from "./loshu.js";
import { renderNatalWheelSVG } from "./natalwheel.js";
import { calculatePlanetaryHours } from "./planetaryhours.js";
import { audioSynthesizer } from "./audio.js";
import { calculateBusinessBlueprint, calculateAddressNumerology } from "./businessengine.js";
import { consultOracle } from "./oracle.js";
import { getSavedProfiles, saveProfile, deleteProfile, generateMarkdownReport, copyToClipboard } from "./export.js";
...
function handleSaveCurrentProfile() {
  ...
  updateSavedProfilesCount();
  runCalculation();
  showToast(`✓ Profile for "${birthName}" saved to your dossiers!`, 'success');
}
...
function openSavedProfilesModal() {
  ...
  actionsBar.querySelector('#btn-modal-export-json').addEventListener('click', () => {
    ...
    downloadAnchor.setAttribute('download', `aetheria_profiles_${Date.now()}.json`);
    ...
    reader.onload = (event) => {
      try {
        ...
        if (Array.isArray(imported)) {
          ...
          saveProfile(combined[0]);
          updateSavedProfilesCount();
          openSavedProfilesModal();
          showToast(`Imported ${imported.length} profiles successfully!`, 'success');
        }
      } catch (err) {
  ...
  if (profiles.length === 0) {
    listContainer.innerHTML = `<div style="text-align: center; color: var(--text-muted); padding: 2rem; background: var(--bg-input); border-radius: var(--radius-md); border: 1px dashed var(--border-subtle);">No saved profiles yet. Click "Save Current Active Profile" above or "Save Profile" on the main form.</div>`;
  } else {
    profiles.forEach(p => {
