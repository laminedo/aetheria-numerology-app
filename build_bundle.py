...
def main():
    with open('js/data/interpretations.js', 'r', encoding='utf-8') as f: interpretations = f.read()
    with open('js/data/presets.js', 'r', encoding='utf-8') as f: presets = f.read()
    with open('js/data/cities.js', 'r', encoding='utf-8') as f: cities = f.read()
    with open('js/data/remedies.js', 'r', encoding='utf-8') as f: remedies = f.read()
    with open('js/data/business.js', 'r', encoding='utf-8') as f: business_data = f.read()
    with open('js/auth.js', 'r', encoding='utf-8') as f: auth_code = f.read()
    with open('js/numerology.js', 'r', encoding='utf-8') as f: numerology = f.read()
    with open('js/astrology.js', 'r', encoding='utf-8') as f: astrology = f.read()
    with open('js/loshu.js', 'r', encoding='utf-8') as f: loshu = f.read()
    with open('js/natalwheel.js', 'r', encoding='utf-8') as f: natalwheel = f.read()
    with open('js/planetaryhours.js', 'r', encoding='utf-8') as f: planetaryhours = f.read()
    with open('js/audio.js', 'r', encoding='utf-8') as f: audio = f.read()
    with open('js/businessengine.js', 'r', encoding='utf-8') as f: businessengine = f.read()
    with open('js/oracle.js', 'r', encoding='utf-8') as f: oracle = f.read()
    with open('js/synthesis.js', 'r', encoding='utf-8') as f: synthesis = f.read()
    with open('js/forecasting.js', 'r', encoding='utf-8') as f: forecasting = f.read()
    with open('js/export.js', 'r', encoding='utf-8') as f: export_js = f.read()

    def strip_es_syntax(text):
        text = re.sub(r'import\s+.*?from\s+[\'"][^\'"]+[\'"];?', '', text)
        text = re.sub(r'import\s+[\'"][^\'"]+[\'"];?', '', text)
        text = re.sub(r'export\s+(async\s+)?(const|let|var|function|class)\s+', r'\1\2 ', text)
    ...
    app_controller = '''
// ==========================================================================
// APPLICATION CONTROLLER & AUTH INTEGRATION
// ==========================================================================
...
function initAuthUI() {
  ...
  if (sigilContainer) {
    ...
    COSMIC_SIGILS.forEach(sig => {
      ...
      btn.addEventListener('click', () => {
        document.querySelectorAll('.sigil-choice-btn').forEach(b => b.classList.remove('selected'));
  ...
  function switchAuthTab(activeTab, activePane) {
    [tabProfile, tabSignin, tabSignup].forEach(t => t && t.classList.remove('active'));
    [paneProfile, paneSignin, paneSignup].forEach(p => p && (p.style.display = 'none'));
    if (activeTab) activeTab.classList.add('active');
    if (activePane) activePane.style.display = 'block';
  }

  if (tabProfile) tabProfile.addEventListener('click', () => switchAuthTab(tabProfile, paneProfile));
  if (tabSignin) tabSignin.addEventListener('click', () => switchAuthTab(tabSignin, paneSignin));
  if (tabSignup) tabSignup.addEventListener('click', () => switchAuthTab(tabSignup, paneSignup));
  ...
  if (formSignup) {
    formSignup.addEventListener('submit', (e) => {
      ...
      try {
        ...
        showToast(`Account created for ${user.name}! Your private vault is ready.`, 'success');
        switchAuthTab(tabProfile, paneProfile);
        closeModal(document.getElementById('auth-modal'));
        runCalculation();
      } catch (err) {
  ...
  if (btnVaultExport) {
    btnVaultExport.addEventListener('click', () => {
  ...
  if (btnVaultImport && vaultFileInput) {
    btnVaultImport.addEventListener('click', () => vaultFileInput.click());
...
function initSolfeggioUI() {
  ...
  SOLFEGGIO_FREQUENCIES.forEach(item => {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = `freq-btn ${item.freq === state.activeTone ? 'active' : ''}`;
    btn.dataset.freq = item.freq;
    btn.innerHTML = `
      <div style="font-weight: 700; font-size: 0.95rem; color: var(--gold-primary); margin-bottom: 2px;">${item.name}</div>
  ...
  audioSynthesizer.onStateChange(({ isPlaying }) => {
    if (btnToggle) btnToggle.classList.toggle('active', isPlaying);
    if (playIcon) {
...
function initEventListeners() {
  ...
  if (presetSelect) {
    presetSelect.addEventListener('change', (e) => {
      ...
      const preset = PRESET_PROFILES.find(p => p.id === selectedId);
      if (preset) {
  ...
  if (btnPythagorean) btnPythagorean.addEventListener('click', () => setSystem('pythagorean'));
  if (btnChaldean) btnChaldean.addEventListener('click', () => setSystem('chaldean'));
  ...
  if (chkSameName) {
    chkSameName.addEventListener('change', (e) => {
      if (inputCurrentName) {
        if (e.target.checked) {
  ...
  if (btnUserAccount) btnUserAccount.addEventListener('click', () => openModal(authModal));

  if (modalCloseBtn) modalCloseBtn.addEventListener('click', () => closeModal(inspectorModal));
  if (savedModalCloseBtn) savedModalCloseBtn.addEventListener('click', () => closeModal(savedProfilesModal));
  ...
  if (btnAskOracle && oracleInput) {
    btnAskOracle.addEventListener('click', () => {
...
function renderSection1Core() {
  ...
  const items = [
    {
      ...
      archetype: CORE_NUMBERS_DATA[b.lifePath.value].title,
      summary: CORE_NUMBERS_DATA[b.lifePath.value].lifePath.summary,
      pills: CORE_NUMBERS_DATA[b.lifePath.value].lifePath.vocations,
      type: 'lifePath',
      isHighlight: true
    },
    {
      ...
      archetype: CORE_NUMBERS_DATA[b.destiny.value].title,
      summary: CORE_NUMBERS_DATA[b.destiny.value].destiny.summary,
      pills: ['Talents: ' + CORE_NUMBERS_DATA[b.destiny.value].destiny.talents.split(',')[0], 'Outer Potential'],
      type: 'destiny'
    },
    {
      ...
      archetype: CORE_NUMBERS_DATA[b.soulUrge.value].title,
      summary: CORE_NUMBERS_DATA[b.soulUrge.value].soulUrge.summary,
    ...
    },
    {
      ...
      archetype: CORE_NUMBERS_DATA[b.personality.value].title,
      summary: CORE_NUMBERS_DATA[b.personality.value].personality.summary,
    ...
    },
    {
      ...
      archetype: CORE_NUMBERS_DATA[b.birthday.value].title,
      summary: CORE_NUMBERS_DATA[b.birthday.value].birthday.talent,
    ...
    },
    {
      ...
      archetype: CORE_NUMBERS_DATA[b.maturity.value].title,
      summary: CORE_NUMBERS_DATA[b.maturity.value].maturity.synthesis,
  ...
  items.forEach(item => {
      ...
      <div>
        ...
        <div class="card-body">
          ...
          <div class="card-pill-list">
            ${item.pills.map(p => `<span class="card-pill">${p}</span>`).join('')}
          </div>
        </div>
      </div>
    ...
    card.querySelector('.btn-inspect').addEventListener('click', () => openFormulaInspector(item.type));
...
function renderSection2Advanced() {
  ...
  if (loshuContainer && loshu) {
    ...
    LOSHU_LAYOUT.forEach(row => {
      ...
      row.forEach(cellNum => {
        ...
        gridHtml += `
          <td class="${isPresent ? 'present' : 'empty'}">
    ...
    loshu.arrowsPresent.forEach(a => {
      arrowsHtml += `
        <div class="arrow-item-card">
    ...
    loshu.arrowsMissing.forEach(a => {
      arrowsHtml += `
        <div class="arrow-item-card loss">
  ...
  if (epochsGrid) {
    ...
    epochs.forEach(ep => {
      ...
      el.innerHTML = `
        <div class="pinnacle-item-header">
          <div>
  ...
  if (attBalContainer) {
    attBalContainer.innerHTML = `
      <div class="blueprint-card">
        <div class="card-top">
          <div class="card-meta">
            <span class="card-tag">First Impressions & Instinct</span>
            <h3>Attitude / Sun Number</h3>
            <div class="card-archetype">${CORE_NUMBERS_DATA[adv.attitude.value].title}</div>
          </div>
          <div class="number-glyph-box">${adv.attitude.value}</div>
        </div>
        <div class="card-body">
          <p>How you instinctively approach life and react when sudden challenges arise. Vibration <strong>${adv.attitude.value}</strong> approaches obstacles with ${CORE_NUMBERS_DATA[adv.attitude.value].keyword.toLowerCase()}.</p>
        </div>
      </div>
      ...
      <div class="blueprint-card">
        <div class="card-top">
          <div class="card-meta">
            ...
            <div class="card-archetype">${CORE_NUMBERS_DATA[adv.balance.value].title}</div>
          </div>
          <div class="number-glyph-box">${adv.balance.value}</div>
        </div>
        <div class="card-body">
          <p>Derived from the initials of your full name (<strong>${adv.balance.initials.map(i => i.char).join('')}</strong>). When emotional turbulence strikes, you regain stability through ${CORE_NUMBERS_DATA[adv.balance.value].keyword.toLowerCase()}.</p>
        </div>
      </div>
    `;
  }
...
function renderSection3NameShift() {
  ...
  if (!state.blueprint.hasNameShift) {
    container.innerHTML = `
      <div class="console-card" style="text-align: center; padding: 3rem 2rem;">
  ...
  container.innerHTML = `
      ...
      <table class="comparison-table">
        ...
        <tbody>
          <tr>
            ...
            <td class="number-cell">${birthExp} (${CORE_NUMBERS_DATA[birthExp].title})</td>
            <td class="number-cell">${currExp} (${CORE_NUMBERS_DATA[currExp].title})</td>
            <td>
              <span class="shift-badge ${ns.destinyDiff ? 'changed' : 'steady'}">
                ${ns.destinyDiff ? `Shift to ${currExp}` : 'Unchanged'}
              </span>
            </td>
          </tr>
          <tr>
            ...
            <td class="number-cell">${birthSU} (${CORE_NUMBERS_DATA[birthSU].title})</td>
            <td class="number-cell">${currSU} (${CORE_NUMBERS_DATA[currSU].title})</td>
            <td>
              <span class="shift-badge ${ns.soulUrgeDiff ? 'changed' : 'steady'}">
                ${ns.soulUrgeDiff ? `Shift to ${currSU}` : 'Unchanged'}
              </span>
            </td>
          </tr>
          <tr>
            ...
            <td class="number-cell">${birthPers} (${CORE_NUMBERS_DATA[birthPers].title})</td>
            <td class="number-cell">${currPers} (${CORE_NUMBERS_DATA[currPers].title})</td>
            <td>
              <span class="shift-badge ${ns.personalityDiff ? 'changed' : 'steady'}">
                ${ns.personalityDiff ? `Shift to ${currPers}` : 'Unchanged'}
              </span>
            </td>
          </tr>
        </tbody>
    ...
    <div class="blueprint-card" style="margin-top: 1.5rem;">
      ...
      <p style="margin-bottom: 0.75rem;">
        Transitioning from <strong>"${state.blueprint.birthName}"</strong> to <strong>"${state.blueprint.currentName}"</strong> creates a noticeable vibrational realignment in your daily reality:
      </p>
      <ul style="padding-left: 1.25rem; font-size: 0.9rem; color: var(--text-secondary); line-height: 1.7;">
        <li>
          <strong>Expression Shift (${birthExp} ➔ ${currExp}):</strong> You are channeling your worldly output through the lens of ${CORE_NUMBERS_DATA[currExp].title} (${CORE_NUMBERS_DATA[currExp].destiny.summary}).
        </li>
        <li>
          <strong>Soul Urge Shift (${birthSU} ➔ ${currSU}):</strong> Your private emotional comfort now resonates strongly with ${CORE_NUMBERS_DATA[currSU].soulUrge.drivers.toLowerCase()}.
        </li>
        <li>
          <strong>Social Projection (${birthPers} ➔ ${currPers}):</strong> The outer world now perceives you through the aura of ${CORE_NUMBERS_DATA[currPers].personality.projection.toLowerCase()}.
        </li>
      </ul>
    </div>
  `;
...
function renderSection4Forecasting() {
  ...
  if (heroGrid) {
    heroGrid.innerHTML = `
      <div class="forecast-hero-card featured">
        <div>
  ...
  if (transitContainer) {
    ...
    roadmap.months.forEach(m => {
      ...
      card.innerHTML = `
        <div class="transit-month-header">
          <div>
...
function renderSection5AstroSynthesis() {
  ...
  if (container) {
    container.innerHTML = `
      <div class="astro-card">
        <div class="astro-glyph-hero">${astro.sunSign.symbol}</div>
        <span class="card-tag">Solar Core & Life Path (${bp.core.lifePath.value})</span>
        <h3>${sunSynth.title}</h3>
        <p style="margin: 0.5rem 0;">${sunSynth.text}</p>
        <div class="card-pill-list" style="margin-top: 0.75rem;">
          <span class="card-pill">Sun in ${astro.sunSign.name}</span>
          <span class="card-pill">${astro.sunSign.element} Element</span>
        </div>
      </div>

      <div class="astro-card">
        <div class="astro-glyph-hero">${astro.moonData && astro.moonData.sign ? astro.moonData.sign.symbol : '☽'}</div>
        <span class="card-tag">Lunar Subconscious & Soul Urge (${bp.core.soulUrge.value})</span>
        <h3>${moonSynth.title}</h3>
        <p style="margin: 0.5rem 0;">${moonSynth.text}</p>
        <div class="card-pill-list" style="margin-top: 0.75rem;">
          <span class="card-pill">Moon in ${astro.moonData.sign.name} (~${astro.moonData.degree}°)</span>
        </div>
      </div>

      <div class="astro-card">
        <div class="astro-glyph-hero">${astro.ascendantData && astro.ascendantData.available ? astro.ascendantData.sign.symbol : '🌅'}</div>
        <span class="card-tag">Ascendant & Personality (${bp.core.personality.value})</span>
        <h3>${ascSynth.title}</h3>
        <p style="margin: 0.5rem 0;">${ascSynth.text}</p>
        <div class="card-pill-list" style="margin-top: 0.75rem;">
          ${astro.ascendantData && astro.ascendantData.available 
            ? `<span class="card-pill">${astro.ascendantData.sign.name} Rising (${astro.ascendantData.degree}°)</span>`
  ...
  if (mansionCard) {
    const moonDeg = astro.moonData.totalLongitude || 0;
  ...
  if (meterGroup) {
    const elem = astro.elemental.percentages;
...
function renderPlanetaryHours() {
  ...
  banner.innerHTML = `
    ...
    <div class="planetary-hours-schedule" style="flex: 2;">
      ${hoursData.schedule.slice(0, 8).map(h => `
        <div class="hour-step-badge ${h.isCurrent ? 'current' : ''}">
          <div>${h.timeLabel}</div>
...
function renderSection6Remedies() {
  ...
  if (grid) {
    grid.innerHTML = `
      <div class="blueprint-card highlight">
        <div class="card-top">
          <div class="card-meta">
        ...
        <div class="card-body">
          ...
          <div class="card-pill-list">
            ${remedies.crystals.map(c => `<span class="card-pill" style="color: var(--gold-primary); font-weight: 600;">${c}</span>`).join('')}
          </div>
    ...
    <div class="blueprint-card highlight">
      ...
      <div style="margin-top: 0.75rem;">
        ...
        <div class="card-pill-list" style="margin-top: 0.35rem;">
          ${result.bestIndustries.map(ind => `<span class="card-pill">${ind}</span>`).join('')}
        </div>
      </div>
    </div>
  `;
...
function handleAskOracle(questionText) {
  ...
  container.innerHTML = `
    <div style="text-align: center; padding: 2rem; color: var(--gold-primary);">
...
function handleCalculateCompatibility() {
  ...
  if (lp1 === lp2) {
    ...
    analysis = `Both carry Life Path ${lp1}. You share an identical cosmic wavelength, creating immediate understanding and shared worldview.`;
  ...
  if (container) {
    container.innerHTML = `
      <div class="blueprint-card highlight" style="margin-top: 1.5rem;">
...
function openSavedProfilesModal() {
  ...
  actionsBar.innerHTML = `
    <button type="button" class="btn btn-primary btn-sm" id="btn-modal-save-current">
  ...
  actionsBar.querySelector('#btn-modal-save-current').addEventListener('click', () => {
    handleSaveCurrentProfile();
    openSavedProfilesModal();
  });

  actionsBar.querySelector('#btn-modal-export-json').addEventListener('click', () => {
  ...
  actionsBar.querySelector('#btn-modal-import-json').addEventListener('click', () => {
    fileInput.click();
  });
    ...
    reader.onload = (event) => {
      try {
        ...
        if (Array.isArray(imported)) {
          imported.forEach(p => saveProfile(p));
          updateSavedProfilesCount();
          openSavedProfilesModal();
  ...
  if (profiles.length === 0) {
    ...
    profiles.forEach(p => {
      ...
      item.querySelector('.btn-del-profile').addEventListener('click', () => {
        deleteProfile(p.id);
        updateSavedProfilesCount();
        openSavedProfilesModal();
...
function openFormulaInspector(type) {
  ...
  if (type === 'lifePath') {
    ...
    const filteredLetters = analysis.lettersData.filter(l => {
    ...
    html = `
      <h3>${title} (${bp.system.toUpperCase()})</h3>
...
(() => {{
  ...
  {strip_es_syntax(interpretations)}
  {strip_es_syntax(presets)}
  {strip_es_syntax(cities)}
  {strip_es_syntax(remedies)}
  {strip_es_syntax(business_data)}
  {strip_es_syntax(auth_code)}
  {strip_es_syntax(numerology)}
  {strip_es_syntax(astrology)}
  {strip_es_syntax(loshu)}
  {strip_es_syntax(natalwheel)}
  {strip_es_syntax(planetaryhours)}
  {strip_es_syntax(audio)}
  {strip_es_syntax(businessengine)}
  {strip_es_syntax(oracle)}
  {strip_es_syntax(synthesis)}
  {strip_es_syntax(forecasting)}
  {strip_es_syntax(export_js)}
...
"""
    ...
    print('SUCCESS! Bundle created with Auth, size in bytes:', len(bundle_code))
